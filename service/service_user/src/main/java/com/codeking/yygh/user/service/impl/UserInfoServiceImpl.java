...
import com.codeking.yygh.enums.AuthStatusEnum;
...
import com.codeking.yygh.user.service.PatientService;
...
public class UserInfoServiceImpl extends ServiceImpl<UserInfoMapper, UserInfo> implements UserInfoService {
    ...
    public Map<String, Object> login(LoginVo loginVo) {
        ...
        if (null == userInfo) {
            ...
            //获取会员
            try {
                // 之前数据库写错了，存了两个，所以需要取第一个，这是一个bug,不打算修复了
                userInfo = baseMapper.selectList(queryWrapper).get(0);
            } catch (Exception e) {
                e.printStackTrace();
            }
            if (null == userInfo) {
